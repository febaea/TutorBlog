...
async function loadPosts() {
  ...
  for (let i = 0; i < post_data.length; i++) {
    let author = htmlEscape(post_data[i].username);
    let timestamp = htmlEscape(post_data[i].created_at);
    let title = htmlEscape(post_data[i].title);
    let content = htmlEscape(post_data[i].content);
