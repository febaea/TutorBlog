...
function displayTutorProfile() {
    ...
    container.innerHTML = `
        <section style="text-align: left;">
            <h2 style="color: #f56d36;">${currentTutor.name}</h2>
            ...
            <div style="margin: 10px 0; font-size: 20px;"> ${currentTutor.rating} / 5.0</div>
            
            <h3 style="margin-top: 25px; color: #f56d36;"> Achievements</h3>
            <ul style="margin: 10px 0 10px 20px;">
                ${currentTutor.achievements.map(a => `<li style="margin: 5px 0;">${a}</li>`).join('')}
            </ul>
            
            <h3 style="margin-top: 25px; color: #f56d36;"> Qualifications</h3>
            <ul style="margin: 10px 0 10px 20px;">
                ${currentTutor.qualifications.map(q => `<li style="margin: 5px 0;">${q}</li>`).join('')}
            </ul>
...
function displayTimetable() {
    ...
    currentTutor.availability.forEach(slot => {
        const isBooked = bookings.some(b => b.tutorId == currentTutor.id && b.timeSlot === slot);
...
function openBookingModal(timeSlot) {
    ...
    bookingDetails.innerHTML = `
        <strong>Tutor:</strong> ${currentTutor.name}<br>
        <strong>Subject:</strong> ${currentTutor.subject}<br>
